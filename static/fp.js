  ...
  Fingerprint2.prototype = {
    ...
    get: function (done) {
      ...
      this.fontsKey(keys, function (newKeys) {
        ...
        that.each(newKeys.data, function (pair) {
          ...
          if (pair.value !== null && typeof pair.value.join !== 'undefined') {
    ...
    timezoneOffsetKey: function (keys) {
      if (!this.options.excludeTimezoneOffset) {
        try{keys.addPreprocessedComponent({key: 'timezone_offset', value: new Date().getTimezoneOffset()});} catch(e){}
      }
      return keys
    },
    ...
    canvasKey: function (keys) {
      if (!this.options.excludeCanvas && this.isCanvasSupported() && this.getDoNotTrack() !== 1) {
        keys.addPreprocessedComponent({key: 'canvas', value: this.getCanvasFp()})
      }
      return keys
    },
    webglKey: function (keys) {
      if (!this.options.excludeWebGL && this.isWebGlSupported() && this.getDoNotTrack() !== 1) {
    ...
    getRegularPlugins: function () {
      ...
      return this.map(plugins, function (p) {
        var mimeTypes = this.map(p, function (mt) {
          return mt !== null ? [mt.type, mt.suffixes].join('~'):'';
        }).join(',');
    ...
    getCanvasFp: function () {
      ...
      ctx.rect(0, 0, 10, 10);
      ctx.rect(2, 2, 6, 6);
      try { result.push('canvas winding:' + ((ctx.isPointInPath(5, 5, 'evenodd') === false) ? 'yes' : 'no'));} catch(e){result.push('canvas winding:no');}
      ...
      try{ result.push('canvas fp:' + canvas.toDataURL()); } catch(e){}
      return result.join('~')
    },
    ...
    getWebglFp: function () {
      ...
      try{ result.push('extensions:' + gl.getSupportedExtensions().join(';')); } catch(e){}
    ...
    getHasLiedOs: function () {
      var userAgent = typeof navigator.userAgent !== 'undefined' ? navigator.userAgent.toLowerCase() : '';
      var oscpu = navigator.oscpu;
      var platform = typeof navigator.platform !== 'undefined' && typeof navigator.platform.toLowerCase !== 'undefined' ? navigator.platform.toLowerCase(): '';
      var os;
    ...
    getHasLiedBrowser: function () {
      if(typeof navigator !== 'object' && typeof navigator.userAgent !== 'object'){
        return false;
      }
...
var details = {};
if (typeof(document.stat) !== undefined) {
  try {
    details = JSON.parse(document.stat);
  } catch (e) {
  }
}

details['s_connection_by_https'] = document.location.protocol === 'https:' ? 1 : 0;

var data = new FormData();
var xhr = new XMLHttpRequest();

// выявление правильного источника
var cookie_duration = 365*24;
var referer = getCookie("r");
var real = document.location.host;
var same = 0;
if(details.s_referer == null || details.s_referer.length === 0) {
  details.s_referer = real;
} else if(details.s_referer != real){
  same = 1;
}
// если кука пустая, ставим ее как реферер пхп
if(typeof(referer) === 'undefined' || referer == 'null' || referer.length == 0) {
  setCookie("r",details.s_referer, cookie_duration);
} else if(referer != details.s_referer) {
  // если реферер от пхп и кука не совпадают и реферер пхп не совпадает с урлом , тогда ставим в куку реферер от пхп
  // (сменился реферер)
  if(same == 1) {
    setCookie("r", details.s_referer, cookie_duration);
    // если реферер пхп совпал с урлом, а кука не совпала, значит меняем реферера пхп на куку и обновляем время куки
  } else {
    details.s_referer = referer;
    setCookie("r", referer, cookie_duration);
  }
  // если реферер пхп совпадает с кукой, обновляем время куки
} else {
  setCookie("r", referer, cookie_duration);
}

if (getCookie("fp").length !== 32) {
  try{
    var fp = new Fingerprint2();
    fp.get(function (result, components) {
      details['f_fingerprint'] = result;
      setCookie("fp", result, 5);
      for (var index in components) {
        var obj = components[index];
        if(obj.value !== null && obj.key !== null){
          if (obj.key !== "canvas" && obj.key !== "webgl") {
            details["f_" + obj.key] = obj.value.toString();
          } else {
            details["f_" + obj.key] = obj.value.toString().substr(0, 100);
          }
        }
      }
      data.append('data', JSON.stringify(details));
      xhr.open('POST', '//stat-ch.storeland.ru/', true);
      xhr.send(data);
    });
  } catch(e){
  ...
  setCookie("fp", getCookie("fp"), 5);
  details['f_user_agent'] = '';
  details['f_fingerprint'] = getCookie("fp");
  data.append('data', JSON.stringify(details));
  xhr.open('POST', '//stat-ch.storeland.ru/', true);
  xhr.send(data);
}